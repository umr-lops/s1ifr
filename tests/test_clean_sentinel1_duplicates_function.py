...
def test_get_ending_processing_time_success():
    ...
    with patch("os.path.isfile", return_value=True), \
        ...
        expected_pattern = "./" + "/metadataObject/metadataWrap/xmlData/{http://www.esa.int/safe/sentinel-1.0}processing"
...
def test_latest_safe_processed():
    ...
    with patch("s1ifr.clean_sentinel1_duplicates_function.get_ending_processing_time", side_effect=dates):
...
def test_spot_dupli_core():
    ...
    mock_walk = [("/archive", ["S1A_IW_SLC__1SDV_20230101_V1.SAFE", "S1A_IW_SLC__1SDV_20230101_V2.SAFE"], [])]
    
    with patch("os.walk", return_value=mock_walk):
        with patch("s1ifr.clean_sentinel1_duplicates_function.latest_safe_processed", 
...
def test_check_duplicate_dryrun():
    ...
    with patch("s1ifr.clean_sentinel1_duplicates_function.spot_dupli_core", return_value=mock_results):
        # FIX: Patch the function where it is USED (in your script), not where it is defined
        with patch("s1ifr.clean_sentinel1_duplicates_function.quarantine_ticket") as mock_quarantine:
