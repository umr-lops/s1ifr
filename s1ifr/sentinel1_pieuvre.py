...
#     unziped_safe = full_path_safe.strip('.tar')
    ...
    doom_flag = NORMAL
    baba = os.path.basename(unzipped_safe)

    is_ok_safe = SAFE_test(unzipped_safe,logpath='/tmp/sentinel1_quality_check_after_unzip.txt',security_time=0)
    ...
    if os.path.exists(original_full_path)==True:
...
def sort_one_safe(full_path_safe,log_file_handler=None,other_archive='mpc',security_second=600):
    ...
    if os.path.exists(full_path_safe):
        if log_file_handler is None:
            logging.debug('sentinel1_pieuvre | note that the SAFE you want to sort and store will not be log')
        ...
        flag_continue,existing_storage,archive = test_existance_of_product(safe_basename,full_path_safe)
        if flag_continue ==True:
            ...
            logging.debug('%1.2fseconds elapsed since file downloaded',seconds_since_creation.total_seconds())
            #second security
            taille1 = os.path.getsize(full_path_safe)
            time.sleep(1)
            taille2 = os.path.getsize(full_path_safe)
            if seconds_since_creation >= datetime.timedelta(seconds=security_second) and taille1==taille2:
                if log_file_handler is not None:
                    log_file_handler.write(full_path_safe+'\n')
                if full_path_safe[-4:]=='.tar':
                    logging.debug('classical case',)
                    spool_dir = WhichSpoolDir(safe_basename,archive=other_archive)
                    os.chdir(spool_dir)
                    st = os.system('tar xf '+full_path_safe)
                    unziped_safe = full_path_safe.strip('.tar')
                    if st != 0 or os.path.exists(unziped_safe)==False:
                        logging.error('uncompress operation on %s failed status = %s',full_path_safe,st)
...
#                         os.system('mv -f '+full_path_safe+' '+whichquarantinedir(safe_basename,archive=other_archive))
                        ...
                        if safe_basename[0:2]=='S1':
                            if not '.SAFE' in unziped_safe:
                                unziped_safe += '.SAFE'
                        doom_flag = finalize_archiving(archive_dir,unziped_safe,final_place,'.tar',archive_name=other_archive)
...
#                     cmd = 'unzip -o -f -v '+full_path_safe+' -d '+spool_dir+'/'
                    ...
                    if safe_basename[0:2]=='S1':
                        if not '.SAFE' in unziped_safe:
                            unziped_safe += '.SAFE'
                    logging.debug('unziped_safe: %s exist=%s',unziped_safe,os.path.exists(unziped_safe))
                    if os.path.exists(unziped_safe):
                        doom_flag = finalize_archiving(archive_dir,unziped_safe,final_place,'.zip',archive_name=other_archive)
                    else:
...
#                         os.system('mv -f '+full_path_safe+' '+whichquarantinedir(safe_basename,archive=other_archive))
                    ...
                    doom_flag = finalize_archiving(archive_dir,unziped_safe,final_place,archive_name=other_archive)
                elif 'tar.' in full_path_safe:
                    logging.debug('sentinel1_pieuvre | remove %s downloaded multiple time in spool',full_path_safe)
#                     shutil.move(full_path_safe,QUARANTINE)
#                     os.system('mv -f '+full_path_safe+' '+whichquarantinedir(safe_basename))
                    quarantine_ticket(full_path_safe,other_archive)
                    doom_flag = QUARANTINED
                else:
                    logging.error('sentinel1_pieuvre | the input : %s is not conventional',full_path_safe)
#                 if level != '2' and mode != 'WV':
                if safe_basename[0:2]=='S1' and doom_flag==NORMAL: #specific behavior for sentinel-1 data
                    inst = ExplodeSAFE(safe_basename)
                    level = inst.get('level')
                    mode = inst.get('mode')
                    kind = inst.get('kind')
                ...
                doom_flag = TOORECENT
                logging.debug('sentinel1_pieuvre | %s has been download too recently (it will be treated at next cron execution)',full_path_safe)
        else:
            doom_flag = ALREADY
...
def main():
    ...
    parser.add_argument("-i", "--input-safe",
