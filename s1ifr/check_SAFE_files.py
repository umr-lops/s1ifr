...
"""
import os
# from __future__ import unicode_literals # link to issue: argument 1 of type 'char const *
import fnmatch
import logging
...
def DeleteCorruptedSAFE(corrupted_list,dirout):
    """

    :param corrupted_list:
    :param dirout:
    :return:
    """
    logging.info('read %s to delete spoted corrupted SAFE',corrupted_list)
    current_time = datetime.datetime.strftime(datetime.datetime.now(),'%Y%m%d_%Hh%M')
    logout = dirout+'deleted_'+current_time+'.lst'
    fod = open(corrupted_list,'r')
    lines = fod.readlines()
    fod.close()
    cpt = 0
    if len(lines)>0:
        ...
        for line in lines:
            ...
            if os.path.exists(safe_path):
                try:
                    shutil.rmtree(safe_path) #we do not remove anything, only # feb2019 too much data: we need to remove corrupted SAFE (taking the risk of fake positive cases)
#                     os.system('/bin/mv -f '+safe_path+' '+QUARANTINE) #put in quarantine rather than deletion
                except:
                    logging.info('error during the suppression of the SAFE: %s',traceback.format_exc())
...
def checkNumberOfMeasurment(manifestpath):
    ...
    try:
        ...
        for occur in meas:
            ...
            if os.path.exists(meas_path) == False:
...
#     dir_name = os.path.dirname(manifestpath)
    ...
    for jj,cc in enumerate(check_sums_values):
        ...
        if md5 != md5_official:
            flag = False
            logging.debug('corruption checksum %s path %s, md5 got %s',md5_official,dat_file_path,md5)
...
def log_path():
    ...
    list_safe_having_problem = dirsuspect+'list_safe_having_problem_'+now_character+'.lst'
    return list_safe_having_problem

def MainLoop(suppression_flag,repdata=None,pattern=None,unique_safe=None,enable_checksum=False,list_safe_having_problem=None,limit_nb_safe=None):
                ...
                if os.path.isdir(safe_path):
                    ...
                    if limit_nb_safe:
                        if counters_internal['is_dir']>limit_nb_safe:
                            logging.info('premature by purpose return with limit_nb_safe %s',limit_nb_safe)
