...
def quarantine_ticket(safe_full_path,other_archive):
    ...
    path_ticket = os.path.join(whichquarantinedir(safe_basename,archive=other_archive),safe_basename)
...
def test_quarantine_before_download(safe_full_path,archive)->bool:
    ...
    associated_potential_quarantine_ticket = os.path.join(whichquarantinedir(safe,archive=archive),safe)
...
def main():
    ...
    parser.add_argument('--inputsafe', action='store', required=True,help='SAFE path to sort (could be SAFE.zip)')
